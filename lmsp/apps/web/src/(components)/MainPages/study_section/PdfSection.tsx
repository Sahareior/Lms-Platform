...
const PdfCard = ({
  ...
  return (
    <article
      ...
      className={`group flex cursor-pointer flex-col rounded-xl p-4 transition-all duration-300 focus:outline-none focus-visible:ring-2 ${
        isDark
    ...
    >
      ...
      <h3
        className={`mb-1.5 line-clamp-2 text-[15px] leading-snug transition-colors ${
          isDark
      ...
      <div
        className={`mt-auto flex items-center justify-between border-t pt-3 text-[11px] ${
          isDark ? "border-[#23262D] text-[#6B7280]" : "border-[#d8d4cb] text-[#4a4a4a]"
        }`}
      >
...
const PdfCardSkeleton = ({ isDark }: { isDark: boolean }) => (
  ...
  >
    <div className="mb-3 flex items-start justify-between">
      <div
        className={`h-11 w-11 animate-pulse rounded-lg ${
          isDark ? "bg-[#23262D]" : "bg-[#e0dcd5]"
        }`}
      />
      <div
    ...
    />
    <div
      className={`mb-2 h-3 w-full animate-pulse rounded ${
        isDark ? "bg-[#23262D]" : "bg-[#e0dcd5]"
      }`}
    />
    <div
      className={`mb-4 h-3 w-5/6 animate-pulse rounded ${
        isDark ? "bg-[#23262D]" : "bg-[#e0dcd5]"
      }`}
    />
...
const PdfSection = () => {
  ...
  const [getSignedPdfUrl] = useGetSignedPdfUrlMutation();
  ...
  // Cloudinary "raw" assets are restricted at the account level — direct URLs
  // return HTTP 401. Ask the backend to generate a short-lived signed delivery
  // URL, then open it. Falls back to the original URL if anything goes wrong.
  const handleOpen = async (pdf: StudyPdf) => {
    if (openingId) return;
    setOpeningId(pdf._id);
    try {
      // 1) Get a signed delivery URL from the backend.
      const { url: signedUrl } = await getSignedPdfUrl({ url: pdf.fileUrl }).unwrap();

      // 2) Open the signed URL directly — it already has the correct content
      //    type so the browser PDF viewer works without a blob workaround.
      window.open(signedUrl, "_blank", "noopener,noreferrer");
      incrementDownload(pdf._id);
    } catch (err) {
      console.error("PDF open failed:", err);
      // Fallback: try fetching as blob (works if account restrictions are lifted).
      try {
        ...
        // Last resort: open raw URL and let the browser/user deal with it.
      ...
      <div className="mx-auto max-w-8xl">
        ...
        <div className="mb-6 flex items-start gap-3">
          <button
            ...
            className={`inline-flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg transition ${
              isDark
          ...
          {tabs.map((tab) => {
                ...
                >
                  ...
                  <div className="relative z-10 flex-1 min-w-0">
                    ...
                    <p
                      className={`mt-0.5 text-[11px] ${
                        active ? "text-[#A1A8B3]" : "text-[#6B7280]"
                      }`}
                    >
                      {tab.subtitle}
                    </p>
                  ...
                  <div
                    className={`absolute bottom-0 left-0 right-0 h-0.5 transition-transform duration-300 ${
                      active ? "bg-[#2F80ED] scale-x-100" : "bg-transparent scale-x-0"
                    }`}
                  />
                </button>
              );
            ...
            return (
              <button
                key={tab.key}
                onClick={() => navigate(`/study-section/pdf/${tab.key}`)}
                className={`group relative flex items-center gap-4 overflow-hidden rounded-lg border-2 border-[#1a1a1a] p-4 text-left transition-all duration-300 ${
                  active
              ...
              >
                ...
                <div
                  className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-lg border-2 border-[#1a1a1a] transition-all ${
                    active
                ...
                <div className="relative z-10 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-base tracking-tight font-black font-serif ${
                        active
                          ? "text-[#b91c1c]"
                  ...
                  <p
                    className={`mt-0.5 text-[11px] font-serif ${
                      active ? "text-[#333]" : "text-[#4a4a4a]"
                    }`}
                  >
                    {tab.subtitle}
                  </p>
                ...
                <div
                  className={`flex h-7 w-7 items-center justify-center rounded-md border-2 border-[#1a1a1a] transition-all ${
                    active
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div
            className={`flex flex-1 items-center gap-2 rounded-lg px-3 py-2.5 sm:max-w-sm ${
              isDark
        ...
        {isLoading ? (
          ...
          >
            <FileText
              size={40}
              className={`mb-3 ${
                isDark ? "text-[#EB5757]/40" : "text-[#b91c1c]/50"
              }`}
            />
            <p
              className={`text-sm ${
                isDark ? "font-semibold" : "font-black font-serif"
              }`}
            >
            ...
            {search && (
              <button
                onClick={() => setSearch("")}
                className={`mt-4 rounded-md px-4 py-2 text-xs transition ${
                  isDark
