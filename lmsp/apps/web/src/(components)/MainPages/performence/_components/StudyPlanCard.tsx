...
export default function StudyPlanCard({
  ...
  if (!isDark) {
    return (
      <div className="bg-[#f2efe9] rounded-2xl border border-[#d8d4cb] p-5 shadow-[2px_2px_0px_0px_#1a1a1a]">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 border-b border-[#d8d4cb] pb-4 gap-2">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#1a1a1a]/5 border border-[#1a1a1a]/20 flex items-center justify-center text-[#1a1a1a] text-sm font-bold shrink-0">
              <Sparkles size={16} />
            </div>
            <div>
              <h3 className="font-bold text-[#1a1a1a] text-lg font-serif">AI স্টাডি প্ল্যান (Study Plan)</h3>
              <span className="text-sm text-[#4a4a4a] block mt-0.5 font-serif italic">
        ...
        {studyPlan.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
            {studyPlan.map((plan, idx) => (
              <div
                key={idx}
                className="bg-[#e8e4db] border border-[#d8d4cb] rounded-xl p-4 shadow-[2px_2px_0px_0px_#1a1a1a] transition flex flex-col justify-between"
              >
                <div>
                  <div className="flex flex-col gap-2 mb-3 items-start">
                    <span className="text-sm font-bold text-[#1a1a1a] bg-[#1a1a1a]/5 px-2.5 py-1 rounded-md border border-[#1a1a1a]/10 inline-block">
                      {plan.day || `দিন ${idx + 1}`}
                    </span>
                    <span
                  ...
                  <div className="text-sm md:text-base font-bold text-[#1a1a1a] mb-2 leading-relaxed font-serif">
                    {plan.title}
                  </div>
                  <div className="text-sm text-[#4a4a4a] leading-relaxed mb-4 font-serif italic">
                    {plan.description}
                  </div>
                </div>
                <div className="flex justify-between items-center text-sm text-[#4a4a4a] border-t border-[#d8d4cb] pt-3 mt-auto">
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
            ...
            <p className="text-sm md:text-base text-[#4a4a4a] max-w-md leading-relaxed font-serif italic">
